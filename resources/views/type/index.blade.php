<x-app-layout>
    <x-slot name="header">
        <h2 class="bg-header-title text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('TIPOS') }}
        </h2>
    </x-slot>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <caption class="p-5 text-lg font-semibold text-center text-gray-900 bg-title-table-amenitie dark:text-white dark:bg-gray-800">
                                <div class="flex justify-between">
                                    <h2 class="h2-amenities">Tipos</h2>
                            ...
                            <thead class="sub-title-amenitie text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 text-center">
                                <tr>
                            ...
                            @foreach ($types as $type)
                                <tbody>
                                    <tr class="bg-row-amenitie border-b dark:bg-gray-800 dark:border-gray-700 text-center">
                                        <th scope="row" class="px-6 py-4 bg-row-amenitie text-gray-900 whitespace-nowrap dark:text-white">
                                            {{$type -> id}}
                                        </th>
                                        <td class="px-6 py-4">
                                            {{$type -> name}}
                                        ...
                                        <td class="px-6 py-4">
                                            <a href="{{ route('type.put', $type->id) }}" class="bg-row-amenitie text-blue-600 dark:text-blue-500 hover:underline">
                                                Actualizar
                                            </a>
                                        </td>
                                        <td class="px-6 py-4">
                                            <form action="{{ Route('type.delete',$type) }}" method="post">
                                                @csrf
                                                @method('delete')
                                                <button class="bg-row-amenitie text-blue-600 dark:text-blue-500 hover:underline">ELiminar</button>
                                            </form>
                                        </td>
                                    </tr>
                                </tbody>
